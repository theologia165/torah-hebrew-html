...
def main():
    if len(sys.argv)!=2:
        raise SystemExit("usage: notion_delivery.py ver2/published/NNN/current.enriched.json")
    ...
    if not token or not parent:
        ...
        state["errors"].append("NOTION_TOKEN and/or NOTION_PARENT_PAGE_ID is not configured in GitHub Actions secrets/variables")
    ...
    html_urls=[]; audio_urls=[]
    ch=d["passage"]["chapter"]
    for v in d["verses"]:
        n=v["verse"]
        html_url=f"{pages_base}/{seq}/genesis-{ch}-{n}.html"
        ...
        children += [
            ...
            heading(3,"ヘブライ語"), embed_external(html_url),
            heading(3,"簡易な説明"), paragraph(v["short_commentary"]),
            heading(3,"詳しい解説")
        ]
        children += detailed_blocks(v["detailed_commentary"])

    payload={"parent":{"page_id":parent},"properties":{"title":{"type":"title","title":rt(title_for(d))}},"children":children[:100]}
    page=req("POST","/pages",token,json=payload)
    page_id=page["id"]
    # Notion create-page accepts at most 100 initial children; append the rest in chunks.
    rest=children[100:]
    while rest:
        chunk,rest=rest[:100],rest[100:]
        req("PATCH",f"/blocks/{page_id}/children",token,json={"children":chunk})

    # Re-fetch and verify top-level delivery contract.
    got=[]; cursor=None
    while True:
        path=f"/blocks/{page_id}/children?page_size=100"
        if cursor: path += f"&start_cursor={cursor}"
        r=req("GET",path,token)
        got.extend(r.get("results",[]))
        if not r.get("has_more"): break
        cursor=r.get("next_cursor")
    embeds=[b.get("embed",{}).get("url") for b in got if b.get("type")=="embed"]
    audios=[b.get("audio",{}).get("external",{}).get("url") for b in got if b.get("type")=="audio"]
    missing_html=[u for u in html_urls if u not in embeds]
    ...
    state_path.write_text(json.dumps(state,ensure_ascii=False,indent=2),encoding="utf-8")
    print(json.dumps(state,ensure_ascii=False))
    return 0 if state["status"]=="PASS" else 4
