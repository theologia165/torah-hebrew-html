...
def duration(path):
    p=run(["ffprobe","-v","error","-show_entries","format=duration","-of","default=nokey=1:noprint_wrappers=1",str(path)])
...
def mean_volume_db(path):
    ...
    m=re.search(r"mean_volume:\s*(-?[0-9.]+) dB",p.stderr); return float(m.group(1)) if m else -999.0

def detect_silences(path):
    p=subprocess.run(["ffmpeg","-hide_banner","-nostats","-i",str(path),"-af",f"silencedetect=noise={SILENCE_NOISE_DB}dB:d={SILENCE_MIN_D}","-f","null","-"],text=True,capture_output=True)
    starts=[float(x) for x in re.findall(r"silence_start:\s*([0-9.]+)",p.stderr)]; ends=[float(x) for x in re.findall(r"silence_end:\s*([0-9.]+)",p.stderr)]
...
def resolve_source(data):
    aliyah=json.loads(get_bytes(ALIYAH_URL).decode("utf-8-sig")); p=data["passage"]
    begin,end=f"{p['chapter']}:{p['start_verse']}",f"{p['chapter']}:{p['end_verse']}"; matches=[]
    ...
    parsha,num=matches[0]; base=f"{parsha}-{num}"
    # PocketTorah keeps spaces in label filenames but strips spaces/apostrophes in audio filenames.
    audio_base=re.sub(r"[\s'’]+","",base)
    labels_base=base
    return {"parsha":parsha,"aliyah":num,"base":base,"audio_base":audio_base,"labels_base":labels_base,
...
def main():
    ...
    data=json.loads(Path(sys.argv[1]).read_text(encoding="utf-8")); out=Path(sys.argv[2]); out.mkdir(parents=True,exist_ok=True)
    source_meta=resolve_source(data); source=out/f"{source_meta['audio_base']}_source.mp3"; source.write_bytes(get_bytes(source_meta["audio_url"]))
