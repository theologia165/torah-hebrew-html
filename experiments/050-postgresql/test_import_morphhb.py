...
class ImportMorphHBTests(unittest.TestCase):
    def test_all_selects_exact_canonical_39_book_order(self):
        self.assertEqual(imp.requested_books("all"), imp.BOOK_CODES)
        self.assertEqual(len(imp.BOOK_CODES), 39)

    def test_selected_books_are_deduplicated_in_request_order(self):
        self.assertEqual(imp.requested_books("Exod,Gen,Exod"), ("Exod", "Gen"))
    ...
    def test_parser_accepts_a_non_genesis_book(self):
        xml = b"""<osis><div type='book' osisID='Exod'><chapter osisID='Exod.1'>
        <verse osisID='Exod.1.1'><w lemma='1121' morph='HNcmsa'>ben</w></verse>
        </chapter></div></osis>"""
        verses = imp.parse_book(xml, {}, "Exod")
        self.assertEqual(verses[0].book, "Exod")
        self.assertEqual(verses[0].osis_wlc, "Exod.1.1")

    def test_verse_map_keeps_verse_compatibility_columns_segment_free(self):
        xml = b"""<verseMap>
          <verse wlc='1Kgs.22.44' kjv='1Kgs.22.43!b' type='partial'/>
          <verse wlc='Ps.13.6!a' kjv='Ps.13.5' type='partial'/>
        </verseMap>"""
        self.assertEqual(imp.parse_verse_map(xml), {"1Kgs.22.44": "1Kgs.22.43"})
