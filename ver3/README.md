...
音声の本文正本は当該RUNのJSON1/JSON1.1（WLC）であり、第一正本朗読音源は固定PocketTorahと語位置ラベルとする。節専用MP3 r1を切り出し、目標0.79306語/秒を上限としてr2へ調整する。ただし速度補正は減速のみ（atempo<=1.0）とし、通常朗読より速くしない。次の語のonsetが検出済み無音区間内にある場合は、その無音区間の終了までを前節に保持する。全節・共有境界・実測duration・音量・速度を検証する。
